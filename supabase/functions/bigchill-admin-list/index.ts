// Returns every code with its used-at time and elapsed seconds, gated by a
// shared password - this is a single-family party game, not a system with
// real accounts to manage, so a shared password checked server-side (never
// shipped in the client bundle) is the right amount of protection here. The
// caller still needs a valid anonymous-auth JWT to get past the API
// gateway (same mechanism as every other bigchill-* function), but that
